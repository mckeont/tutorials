...
- sex: Male=1 Female=2
C:\Users\Tom McKeon\Downloads

## What is censoring?
```{r}
pcr <- read.csv("C:/Users/Tom McKeon/Downloads/PCRdata_combined_05282020.csv", header = TRUE)
...
## formatting our pcr data's datefields
```{r}
pcr$DX_day <- "01"
```

```{r}
pcr <- unite(pcr, startdate, c(DXyear, DXmonth, DX_day), remove=FALSE)
```

```{r format_date1}
pcr$DateLastContact <- as.character(pcr$DateLastContact)
```
...
pcr<- pcr %>% 
  mutate(
    startdate = as.Date(startdate, format = "%Y_%m_%d"), 
    DateLastContact = as.Date(DateLastContact, format = "%Y%m%d"),
    )
...
pcr<- pcr %>% 
  mutate(
    os_yrs = 
      as.duration(startdate %--% DateLastContact) / dyears(1)
    )
...
```{r survfunc}
Surv(pcr$os_yrs, pcr$VitalStatus)[1:10]
```
...
```{r lung_survfit}
f1 <- survfit(Surv(os_yrs, VitalStatus) ~ 1, data = pcr)
names(f1)
```
...
```{r}
plot(survfit(Surv(os_yrs, VitalStatus) ~ 1, data = pcr), 
     xlab = "Days", 
...
ggsurvplot(
    fit = survfit(Surv(os_yrs, VitalStatus) ~ 1, data = pcr), 
    xlab = "Days", 
    ylab = "Overall survival probability")
```
...
```{r 5yrest}
summary(survfit(Surv(os_yrs, VitalStatus) ~ 1, data = pcr), times = 365.25)
```
...
```{r}
survfit(Surv(os_yrs, VitalStatus) ~ 1, data = pcr)
```
...
```{r}
survdiff(Surv(os_yrs, VitalStatus) ~ Sex, data = pcr)
```
...
```{r}
sd <- survdiff(Surv(os_yrs, VitalStatus) ~ Sex, data = pcr)
...
```{r}
coxph(Surv(os_yrs, VitalStatus) ~ Sex, data = pcr)
```
...
fit4 <- survfit(Surv(os_yrs, VitalStatus) ~ Sex, data = pcr)
